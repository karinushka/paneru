...
use mach2::message::{
    ...
    MACH_MSGH_BITS_REMOTE_MASK, MACH_RCV_MSG, MACH_RCV_TIMEOUT, MACH_SEND_MSG,
    MACH_SEND_TIMEOUT, mach_msg, mach_msg_body_t, mach_msg_header_t,
    mach_msg_ool_descriptor_t, mach_msg_port_descriptor_t, mach_msg_size_t,
};
    ...
    for _ in 0..body.msgh_descriptor_count {
        ...
        match kind {
            MACH_MSG_OOL_DESCRIPTOR => {
                ...
                let desc = unsafe {
                    std::ptr::read_unaligned(
                        bytes.as_ptr().add(offset).cast::<mach_msg_ool_descriptor_t>(),
                    )
                };
            ...
            MACH_MSG_PORT_DESCRIPTOR => {
                ...
                let desc = unsafe {
                    std::ptr::read_unaligned(
                        bytes.as_ptr().add(offset).cast::<mach_msg_port_descriptor_t>(),
                    )
                };
                if u32::from(desc.disposition) == RECEIVED_SEND
