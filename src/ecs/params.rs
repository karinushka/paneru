...
pub struct SmoothSwipeTracking<'w, 's> {
    swipe_tracker: ResMut<'w, TrackpadSwipe>,
...
impl SmoothSwipeTracking<'_, '_> {
    pub fn on_cooldown(&mut self, commands: &mut Commands) -> bool {
        let TrackpadSwipe::Active { cooldown, .. } = &mut *self.swipe_tracker else {
            return true;
        };
        if *cooldown < 1 {
            return false;
    ...
    pub fn active(&self) -> bool {
        ...
        if let TrackpadSwipe::Active { last_swipe, .. } = self.swipe_tracker.as_ref()
    ...
    pub fn initiate_cooldown(&mut self) {
        ...
        if let TrackpadSwipe::Active { cooldown, .. } = &mut *self.swipe_tracker {
