...
mod tests {
    ...
    fn deferred_deadline_wakes_real_pump_core_and_delivers_event() {
        ...
        let (events, should_exit, did_wait) =
            pump_receiver(&receiver, activity, now, false, |wait| {
                observed_wait = wait;
                sender
                    .send(Event::ApplicationActivated)
        ...
        assert!(
            events
                .iter()
                .any(|event| matches!(event, Event::ApplicationActivated))
        );
    }
    ...
    fn write_synthetic_after_pump(mut written: Local<bool>, mut commands: Commands) {
        ...
        *written = true;
        commands.trigger(SendMessageTrigger(Event::ApplicationActivated));
    }
    ...
    fn first_native_reader(
        ...
        probe.first_reader += messages
            .read()
            .filter(|event| matches!(event, Event::ApplicationActivated))
            .count();
    }
