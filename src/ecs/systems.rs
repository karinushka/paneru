...
use crate::ecs::{
    ...
    ReshuffleAroundMarker, StackAdjustedResize, Unmanaged, WindowDraggedMarker,
...
pub(crate) fn finish_setup(
    ...
    for (mut strip, active_strip) in &mut workspaces {
        ...
        if active_strip
            && let Some(entity) = strip.first().ok().and_then(|column| column.top())
        {
                    ...
                    && initializing.is_none()
                {
                    ...
                    if is_top_edge_drag {
                        ...
                        let above = strip
                            ...
                            .and_then(|column| {
                                if let Column::Stack(ref stack) = column {
                                    let pos = stack.iter().position(|&e| e == entity)?;
                                    (pos > 0).then(|| stack[pos - 1])
                                } else {
                                    None
                                }
                            });

                        if let Some(above_entity) = above {
                            if let Ok((mut above_window, _, _)) =
                                windows.get_mut(above_entity)
                            {
