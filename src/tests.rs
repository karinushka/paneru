...
fn setup_test_process(
    ...
    let windows = (0..5)
        .map(|i| {
            let size = CGSize::new(400.0, 800.0);
...
fn setup_world(app: &mut App, event_queue: &EventQueue) -> MockApplication {
    ...
    let mut display = Display::new(
        ...
        CGRect::new(CGPoint::new(0.0, 0.0), CGSize::new(1024.0, 768.0)),
        20,
    );
...
fn test_window_last() {
    ...
    let mut app = App::new();
    let event_queue = Arc::new(RwLock::new(Vec::<Event>::new()));
    setup_world(&mut app, &event_queue);
